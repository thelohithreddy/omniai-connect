...
addresses live in Better Auth's own schema, which ADR-0014 keeps unreachable from `omniai_app` —
...
- **`VARCHAR(320)`** — byte-identical to `invitations.invited_email`, because it holds the same kind
  ...
  discovered.
- **No foreign key, anywhere**, and in particular none into Better Auth's own schema. ADR-0014
  clause 1 keeps rollback safety structural by ensuring no Alembic migration names that schema at
  all — not even in prose, so the invariant stays greppable — and this migration does not. The
  column is a plain string the workspace's owner typed, never a reference to a user row.
